...
import { certificationsApi } from "@/lib/apiClient";
    ...
    <div className="p-4 lg:p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="card p-4">
          ...
          <p className="text-2xl font-bold text-slate-100">{certs.filter((c) => c.status === "completed").length}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-slate-400 mb-1">In Progress</p>
          <p className="text-2xl font-bold text-brand-blue">{certs.filter((c) => c.status === "in-progress").length}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-slate-400 mb-1">Available</p>
          <p className="text-2xl font-bold text-slate-300">{certs.filter((c) => c.status === "not-started").length}</p>
        </div>
      </div>
